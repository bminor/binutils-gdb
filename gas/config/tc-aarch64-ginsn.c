...
aarch64_ginsn_ldstp (const symbolS *insn_end_sym)
{
  ...
  /* Save / restore of WZR is not of interest for SCFI.  Also, insns like ldpsw
     (marked with subclass F_SUBCLASS_OTHER) do not need to generate any load
     or store for SCFI purposes.  Lastly, for CFI puposes, the width of save /
     restore operation has to be 8 bytes or more.  However, the address
     processing component may have updated the stack pointer.  At least, emit
     that ginsn and return.  Also note, TBD_GINSN_GEN_NOT_SCFI.  */
  if (other_p || aarch64_zero_register_p (opnd1)
  ...
  if (opnd1->qualifier == AARCH64_OPND_QLF_W
    ...
    width = 4;
  else if (opnd1->qualifier == AARCH64_OPND_QLF_S_Q)
    {
      width = 16;
...
aarch64_ginsn_ldstr (const symbolS *insn_end_sym)
{
  ...
  /* Save / restore of WZR is not of interest for SCFI.  Also insns like
     stg, prfm, ldrsw etc. (marked with subclass F_SUBCLASS_OTHER) do not need
     to generate any load / store ginsns for SCFI purposes.  Lastly, for CFI
     puposes, the width of save / restore operation has to be 8 bytes or more.
     That said, the address processing component may have updated the stack
